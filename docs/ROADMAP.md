...
| **Les entités suivent les builds** (`entities/`) | ✅ — même principe : les octets voyagent, seuls les champs qui SITUENT sont réécrits en place. Une copie a de nouveaux `UUID` (hachés sur l'ancien et la position), un déplacement garde les siens ; une entité accrochée suit le bloc qui la porte ; une position RETENUE (lit, ruche, laisse) ne suit que si son bloc suit. Pas vu en jeu ; mesuré sur aucune vraie save — `recenser_entites` le fera |
...
| **Composants** | une définition + N instances. Modifier la définition met à jour les N — ✅ moteur (`tf-ops/src/composant.rs`) et outil « Composant » dans la coque |
...
> l'outil est dans la coque — reste à l'essayer à la souris sur une vraie
> machine.
